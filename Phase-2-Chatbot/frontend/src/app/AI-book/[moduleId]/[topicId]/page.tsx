...
export default function TopicPage() {
  ...
  useEffect(() => {
    const fetchTopicContent = async () => {
      if (moduleId && topicId && typeof moduleId === 'string' && typeof topicId === 'string') {
        try {
          // Fetch the specific topic file using both moduleId and topicId
          const topicParam = topicId.replace(/-/g, '_'); // Convert hyphens back to underscores for URL parameter
          const response = await fetch(`/api/module-content?id=${moduleId}&topic=${topicParam}`);
          ...
          setContent(`# ${topicId.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}\n\nTopic content could not be loaded. This section would contain information about ${topicId.replace('-', ' ')}.`);
        } finally {
          setLoading(false);
        }
      }
    ...
    return (
      <div className="py-6 sm:py-8">
        <div className="max-w-3xl sm:max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-lg sm:text-xl text-gray-700 dark:text-gray-300">Loading topic content...</div>
        </div>
      </div>
    );
  }
        ...
        <header className="mb-6 sm:mb-8">
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 mb-3 sm:mb-4">
            ...
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">
              {typeof topicId === 'string' ? topicId.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase()) : ''}
            </h1>
          </div>
          <div className="flex flex-wrap items-center text-sm text-gray-500 dark:text-gray-400 gap-2">
            <Link href={`/AI-book/${moduleId}`} className="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 truncate max-w-\[200px\] sm\:max-w-\[300px\]">
              {currentModule?.title || moduleId}
            </Link>
            <span className="mx-1">/</span>
            <span className="bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-200 px-2.5 sm:px-3 py-0.5 sm:py-1 rounded-full text-xs sm:text-sm truncate max-w-\[150px\] sm\:max-w-\[200px\]">
              {topicId}
            </span>
          </div>
        </header>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl shadow-md sm:shadow-lg p-5 sm:p-6 md:p-8 mb-6 sm:mb-8 border border-gray-200 dark:border-gray-700">
          <ReactMarkdown
            ...
            components={{
              ...
              a: ({node, ...props}) => <a className="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 underline truncate max-w-full" {...props} />,
